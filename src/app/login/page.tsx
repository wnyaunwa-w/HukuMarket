...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(loginMode === 'phone' ? "Incorrect phone number or PIN." : "Incorrect email or password.");
      console.error(err);
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-[80vh] flex items-center justify-center p-4 py-12">
      <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-xl max-w-md w-full">
        ...
        <p className="text-slate-500 text-center mb-8">
          {loginMode === 'phone' ? "Enter your mobile number and PIN" : "Enter your email and password"}
        </p>
            ...
            <>
             ...
             <>
              ...
              <div>
                ...
                <div className="relative">
                  <Lock className="absolute left-3 top-3 text-slate-400" size={20} />
                  <input type="password" required className="w-full pl-10 p-3 border rounded-xl outline-none focus:ring-2 ring-orange-100 bg-slate-50" placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} />
                </div>
              </div>
            </>
          )}
        ...
        <div className="mt-8 pt-6 border-t border-slate-100 text-center">
          ...
          <div className="space-y-4">
            ...
            >
              {loginMode === 'phone' ? "Log in with Email & Password" : "Log in with Phone & PIN"}
            </button>
          </div>
        </div>
